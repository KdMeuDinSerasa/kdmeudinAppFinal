...
class BillsViewHolder(itemView: View): RecyclerView.ViewHolder(itemView) {
    ...
    fun bind(bill: BillsModel){
        ...
        binding.textViewBillExpireDate.text = "${itemView.context.getString(R.string.Bill_ExpireDate_label_list_card)} ${bill.expire_date.toString().substring(8,10)}/0${bill.expire_date.month + 1}/${bill.expire_date.adjustYear()}"
